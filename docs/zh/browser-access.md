...
Web UI 传输层位于一个**可选的、进程内加载的附加包** `@cortex-agent/ui-server` 中。
核心 agent-server（仅 Slack / TUI）**不**携带 `@trpc/server` 或任何 UI 代码：只有当你
用 `CORTEX_UI_HTTP` 主动开启时，该包及其依赖才进入运行时。标志未设置时该包从不被导入，
因此仅用 Slack 或 TUI 的部署不承担任何 UI 负担。

启用后，`@cortex-agent/ui-server`：
...
`web/dist` 是一个纯静态产物（`index.html` + 带哈希的资源）。它**故意不**包含在发布的
npm 包中——你在部署时从源码构建它。

### 3. `web/dist` 从何处被提供

`@cortex-agent/ui-server` 按以下顺序解析 SPA 目录：

1. 你显式传给它的目录；
2. `CORTEX_UI_SPA_DIR` 环境变量；
3. monorepo 的 `web/dist`，相对于已安装的包解析。

如果你从仓库检出部署，第 3 项无需配置即可生效——就地构建 `web/dist` 便会被自动提供。
如果你把构建后的 SPA 部署到其他位置，用 `CORTEX_UI_SPA_DIR` 指向它。如果目录不存在
（SPA 未构建），非 `/trpc` 路径返回 404 占位，而 `/trpc` 仍可工作。
...
`CORTEX_UI_HTTP` 接受 `1`、`true`、`on` 或 `yes`。未设置时，该端点——以及整个
`@cortex-agent/ui-server` 包——都不会加载。
...
Cloudflare Tunnel   (cortex-ui.example.com  →  服务器 localhost:3004)
  ...
  ▼
@cortex-agent/ui-server  （校验 JWT；浏览器从不持有 clientToken）
  ├─ 提供 web/dist  （同源 SPA）
