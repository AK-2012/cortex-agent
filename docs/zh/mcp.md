...
| `thread_start` | `message`、`template?`、`agent?`、`goal?`、`done_when?`、`deliverable_path?`、`context_files?`、`budget_usd?`、`project?`、`wait?` | 从智能体内启动子线程（子管道） |
| `thread_status` | `threadId` | 查询线程的执行状态（running/waiting/completed/failed/cancelled/aborted） |
| `thread_cancel` | `threadId` | 通过 ID 取消运行中的线程 |
