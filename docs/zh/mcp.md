...
暴露与远程机器交互的工具。这是线程/模板会话加载的唯一服务器——线程智能体获得远程机器访问权限，但不获得 Slack、费用或调度工具。
...
服务器实现在 `agent-server/src/domain/mcp/core-server.ts`。工具实现在 `agent-server/src/domain/mcp/tools/task-ops.ts`。

### cortex-ext

暴露 Cortex 管理工具：调度、费用查询、执行追踪、Slack 文件上传和上下文解析。此服务器仅由直接/用户发起的会话加载——线程智能体不获得这些工具。
...
服务器实现在 `agent-server/src/domain/mcp/server.ts`。各个工具在 `agent-server/src/domain/mcp/tools/` 中：`slack.js`、`cost.js`、`executions.js`、`context.js`、`schedule.js`。
...
Cortex 在启动时自动生成三个 MCP 配置文件（通过 `agent-server/src/core/config-generator.ts` 和 `agent-server/src/entry/startup-helpers.ts` 中的 `ensureMcpConfig()` 调用）：

| 文件 | 加载者 | 服务器 |
|---|---|---|
| `~/.cortex/config/mcp-config.json` | 直接/用户发起的会话 | cortex-core + cortex-ext |
...
- **TUI 模式**：加载 `mcp-config-tui.json`
- **Print 模式，用户发起的会话**：加载 `mcp-config.json`
- **线程/模板会话**：加载 `mcp-config-core.json`

线程会话覆盖发生在 Claude 适配器（`adapter.ts`）中，它检查 `session.cortexContext.useCoreMcp`。此字段由线程执行器设置，确保线程智能体仅获得远程机器工具，不获得 Slack、费用或调度工具。
