...
|---|---|---|
| `event-types.ts` | types | CortexEvent union type (26 user events + 2 meta events; incl. `execution.log` live log-tail stream and `session.message` S4 chat live stream — sessionId/channel/role/text, published at agent-runner's conversation-history append points) + CortexEventInput (distributive Omit) |
