...
export function registerContextTools(server: McpServer, deps: ContextToolDeps): void {
  server.tool(
    'cortex_context',
    'Return the current Cortex execution context: channel, sessionId, sessionName (cortex-XXXX), threadId, profile, project, backend. Use this to discover the current scope before calling cortex_schedule_add with target=current-channel/current-session/current-thread.',
    {},
    { readOnlyHint: true },
    async () => {
      try {
