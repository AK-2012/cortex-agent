...
| `executions/` | [S14] DONE | Execution registry re-export layer (registry.ts) — wraps ExecutionRepo terminal transitions with lock-release side effect. `log-tailer.ts` (342f/B2): ref-counted live tail of a running cortex-run/dispatch output.log → bounded `execution.log` EventBus events (fs-local + bash-remote, no client-protocol change) |
