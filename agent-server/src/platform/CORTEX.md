...
| `adapter.ts` | interface | PlatformAdapter interface + capability declaration |
...
| `virtual-message.ts` | aggregator | VirtualMessage merges multiple appends into fewer messages (default retry delays `[200,600,1500,4000]ms`; test with `_testSetRetryDelays([0,0,0,0])` to skip wall-clock waiting) |
...
| `testing.ts` | testing | MockAdapter in-memory mock implementation |
