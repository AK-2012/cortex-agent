...
| `composite-adapter.ts` | adapter | CompositeAdapter wrapping N sub-adapters (Slack/Feishu/TUI) behind one PlatformAdapter surface; routes outbound by `ownsConduit` prefix; system-notice fans out to real primaries, project-report fans out by binding; FanOutOutputStream; extractTuiAdapter. Fan-out posts aggregate each platform's sub-ref into `MessageRef.parts[]`; `openOutputStream` resolves a **per-platform thread anchor** from `opts.anchorRef.parts` via `ownsConduit` (a Slack ts is not a valid Feishu open_message_id — sharing one anchor caused Feishu 99992354/HTTP 400). Unmatched platforms get `threadId=null` and self-anchor. |
