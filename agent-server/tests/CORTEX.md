...
| `no-trpc-dep.test.ts` | Test | Guard (task 3606): agent-server production `dependencies` + `optionalDependencies` must NOT include `@trpc/server` (it lives in the optional `@cortex-agent/ui-server` package) |
