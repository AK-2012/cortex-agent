...
| `ui-http-wiring.test.ts` | Test | Entry-layer wiring `startUiHttpServer` (entry/start-ui-http): env gate on/off, default port 3004, 127.0.0.1 bind, 401 token gate, HTTP query+mutate roundtrip, SSE one-event, clean close(); **CORS via `CORTEX_UI_CORS_ORIGINS` env** — comma-separated allow-list (trim + drop empties) reaches the transport-host so app.ts honors CORS via env: allow-listed Origin echoes ACAO, preflight OPTIONS→204, non-listed origin/unset env → none — fake UiService, ephemeral port |
