...
2. **Pass** → record the verdict: `cortex-task verdict --project <project> --task-id <your Task ID> --child <child id> --verdict accepted --note "<what you verified>"` (this stops the result from re-delivering to future incarnations), then distill the key conclusions into your artifact; move on.
3. **Fail** → record it: `cortex-task verdict ... --verdict rejected --note "<expected/actual gap>"`, write the gap and your hypothesis into the artifact, then either `cortex-task uncomplete` + edit the child with a sharper contract, or add a revision child via the same `decompose --keep-parent` call. Update your checkpoint (the wait gate requires it), then call `thread_wait` again.
