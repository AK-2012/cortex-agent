...
| `screens/` | Screen slots. **`MobileThreadsScreen` (5b) is REAL** (task ad9c, below). Still STUB (`StubScreen` shared body, 守则11 neutral placeholders replaced by later passes): `MobileSessionsScreen` (5a) · `MobileTasksScreen` (5c) · `MobileMachinesScreen` (机器, 3b 同构) · `MobileApprovalsScreen` (10e) · `MobileOverviewScreen` (10f). |
| `screens/screens.test.tsx` | render checks for the remaining STUB slots (5a/5c/机器/10e/10f). 5b is excluded — it is now a tRPC-bound container needing query providers (covered by `MobileThreadViews.test.tsx` + the live harness). |
