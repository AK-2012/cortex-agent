...
test('query schemas accept valid input', () => {
    ...
    assert.equal(queryInputSchemas['tasks.list'].parse({ projectId: 'p', status: 'open', actionable: true }).status, 'open');
    assert.deepEqual(queryInputSchemas['executions.list'].parse({ status: ['running'], limit: 5 }), { status: ['running'], limit: 5 });
...
test('mutate schemas require their mandatory fields', () => {
    ...
    assert.deepEqual(mutateInputSchemas['tasks.claim'].parse({ projectId: 'p', taskId: 'f184' }), { projectId: 'p', taskId: 'f184' });
    // tasks.block requires reason
    assert.throws(() => mutateInputSchemas['tasks.block'].parse({ projectId: 'p', taskId: 'f184' }));
    assert.deepEqual(mutateInputSchemas['tasks.block'].parse({ projectId: 'p', taskId: 'f184', reason: 'stuck' }), { projectId: 'p', taskId: 'f184', reason: 'stuck' });
    // missing required id
